...
// Playing with IOCC submits is unlikely of having influence to my code...
...
// I justed wanted to try this. Well. Gdb spits on me :-P
#define prepare_swap(ret, tmp, old, new)                 \
	\
if(new != NULL) {                                \
	tmp = strreplace(ret,old,new);           \
...
char * prepare_url(const char *URL, const char *artist, const char *album, const char *title)
{
	...
	// ret gets changed
	prepare_swap(ret,tmp,"%artist%",p_artist);
