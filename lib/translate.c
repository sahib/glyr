...
static const char * gtrans_get_detector_url(const char * input)
{
        ...
        if(input != NULL) {
                char * escape_input = curl_easy_escape(NULL,input,0);
                if(escape_input != NULL) {
                        resultURL = strdup_printf("https://www.googleapis.com/language/translate/v2/detect?key=%s&q=%s",GOOGLE_TRANSLATE_API_KEY,escape_input);
                }
                free(escape_input);
        }
        return resultURL;
...
char ** Gly_list_supported_languages(GlyQuery * s)
{
        ...
        if(s != NULL) {
                ...
                const char * dl_url = "https://www.googleapis.com/language/translate/v2/languages?key="GOOGLE_TRANSLATE_API_KEY"&prettyprint=true";
                if(dl_url != NULL) {
                        GlyMemCache * DCache = download_single(dl_url,s,NULL);
